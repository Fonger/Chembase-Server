    ...
    '||': function (a, b, query) {
        if (query) {
            ...
            if (a instanceof PotentialMatch || b instanceof PotentialMatch)
                return new PotentialMatch(result);
            return result;
        }
        return a || b;
    },
...
const unops = {
    '-': function (a, query) {
        if (query && (a instanceof PotentialMatch || a instanceof CompoundCondition))
            throw new Error('Cannot use with - operator');
        return -a;
    },
    '+': function (a, query) {
        if (query && (a instanceof PotentialMatch || a instanceof CompoundCondition))
...
async function executeArray(list, context, query) {
    return await Promise.all(list.map(async function (v) { return await execute(v, context, query); }));
}
...
async function execute(node, context, query) {
    switch (node.type) {
        case 'ArrayExpression':
            return await executeArray(node.elements, context, query);
        ...
        case 'CallExpression':
            ...
            if (fn.constructor.name === 'AsyncFunction') {
                return await fn.apply(caller, await executeArray(node.arguments, context, query));
            } else {
                return fn.apply(caller, await executeArray(node.arguments, context, query));
            }
        case 'ConditionalExpression':
            return await execute(node.test, context, query)
                ? await execute(node.consequent, context, query)
                : await execute(node.alternate, context, query);
