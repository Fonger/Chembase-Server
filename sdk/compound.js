...
var bson = new BSON();
//require('buffer'); browersify auto load this
...
function Compound () {
    if (arguments.length === 0) {
        ...
        if (typeof compound !== 'object' || compound.constructor !== Object)
            throw new Error('Compoubd must be created from a plain object');
        this._parseData = compound;
        this._rawData = bson.serialize(compound);
    } else if (arguments.length === 2) {
        if (arguments[0].constructor.name !== 'Beaker')
            throw new TypeError('Must specify a valid beaker');
...
Compound.prototype.update = function (compoundData, replace) {
    ...
    var promise = new Promise(function(resolve, reject) {
        self._beaker._lab.socket.emit('update', request, function(result) {
